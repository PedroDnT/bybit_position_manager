...
class PositionRiskManager:
    ...
    def analyze_position_volatility(
        ...
        if side == "long":
            ...
            r_unreal = (
                max(0.0, (entry_price - live_price) / R_unit) if R_unit > 0 else 0.0
            )
        trail_suggestion = compute_trailing_stop(
            entry=live_price,
            direction=side,
            atr=atr,
            cfg=self.cfg,
            r_unrealized=r_unreal,
        )
        ...
        if side == "long":
            optimal_dollar_risk = optimal_position_size * (
                entry_price - risk_params["SL"]
            )
            optimal_dollar_reward = optimal_position_size * (
                risk_params["TP"] - entry_price
            )
            sl_pct = ((risk_params["SL"] - entry_price) / entry_price) * 100
            tp_pct = ((risk_params["TP"] - entry_price) / entry_price) * 100
        else:
            optimal_dollar_risk = optimal_position_size * (
                risk_params["SL"] - entry_price
            )
            optimal_dollar_reward = optimal_position_size * (
                entry_price - risk_params["TP"]
            )
        ...
        if side == "long":
            current_dollar_risk = current_position_size * (
                entry_price - risk_params["SL"]
            )
            current_dollar_reward = current_position_size * (
                risk_params["TP"] - entry_price
            )
        else:
            current_dollar_risk = current_position_size * (
    ...
    def export_to_json(self, filename: str = "risk_analysis.json"):
        ...
        output = {
            ...
            "positions": self.risk_analysis,
            "portfolio": (
                self._calculate_portfolio_metrics() if self.risk_analysis else {}
            ),
            "account": self.account_metrics,
        }
