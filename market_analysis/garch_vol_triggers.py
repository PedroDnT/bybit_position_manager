...
def get_klines_bybit(
    exchange: ccxt.Exchange, symbol="BTC/USDT", timeframe="4h", since=None, limit=1000
):
    """
...
def get_multiple_timeframes_bybit(
    exchange: ccxt.Exchange, symbol="BTC/USDT", timeframes=["4h"], days_back=120
):
    """
...
def har_rv_nowcast(rv_series: pd.Series, *, bars_per_day: int):
    ...
    Xmat = np.column_stack(
        [np.ones(len(df)), df["RV_D"].values, df["RV_W"].values, df["RV_M"].values]
    )
    ...
    X_last = np.array(
        [1.0, last["RV_D"].iloc[0], last["RV_W"].iloc[0], last["RV_M"].iloc[0]]
    )
...
def blended_sigma_h(
    ...
    *,
    ...
    sigH_garch_frac = (
        _ann_to_horizon_sigma(sigma_ann_garch, horizon_hours)
        if sigma_ann_garch
        else None
    )
    sigH_har_frac = (
        _ann_to_horizon_sigma(sigma_ann_har, horizon_hours) if sigma_ann_har else None
    )
    ...
    if (
        sigma_ann_har
        and sigma_ann_garch
        and (sigma_ann_garch > outlier_ratio * sigma_ann_har)
    ):
        w_g = w_g * 0.3
        s = w_g + w_h + w_a
    ...
    if sigH_garch_frac is not None:
        parts_frac.append(sigH_garch_frac)
        weights.append(w_g)
    if sigH_har_frac is not None:
        parts_frac.append(sigH_har_frac)
        weights.append(w_h)
    # ATR available always
    parts_frac.append(sigH_atr_frac)
    weights.append(w_a)
...
def validate_garch_result(
    returns: np.ndarray, res, sigma_ann: float, horizon_hours: int
) -> list[str]:
    issues: list[str] = []
...
def sl_tp_and_size(
    entry_price, sigma_H, k=1.2, m=2.0, side="long", R=100.0, tick_size=None
):
    """
...
def compute_trailing_stop(
    entry: float, direction: str, atr: float, cfg: dict, r_unrealized: float
) -> float:
