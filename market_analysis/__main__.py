...
def main():
    ...
    if manager.account_metrics:
        ...
        print(
            f"Today's Total PnL (Realized + Unrealized): ${metrics.get('todays_total_pnl', 0):,.2f}"
        )
        print("-" * 80)
    print(
        f"{'Symbol':<15} {'Side':<5} {'Entry':<10} {'SL':<10} {'TP':<10} {'R:R':<6} {'Risk%':<7} {'k/m':<9}"
    )
    print("-" * 80)
    ...
    for position in positions:
        ...
        if analysis and "stop_loss" in analysis:
            print(
                f"{symbol:<15} {analysis['side'].upper():<5} "
