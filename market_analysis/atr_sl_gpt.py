...
def compute_levels(entry: float, atr: float, k: float, m: float, side: str) -> Levels:
    ...
    return Levels(
        sl_price=sl, tp_price=tp, sl_distance=sl_dist, tp_distance=tp_dist, rr=rr
    )


def atr_trailing_stop(
    price: float, atr: float, trail_mult: float, side: str, last_trail: Optional[float]
) -> float:
    """
...
def approx_liq_price(
    entry: float, side: str, leverage: float, maint_margin_frac: float = 0.005
) -> float:
    """
...
def fetch_ohlcv_df(
    exchange: ccxt.Exchange, symbol: str, timeframe: str, limit: int = 300
) -> pd.DataFrame:
