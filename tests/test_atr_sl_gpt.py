...
def test_atr_trailing_stop_long():
    ...
    trail_stop = atr_trailing_stop(
        price=1000, atr=20, trail_mult=2.0, side="long", last_trail=None
    )
    assert np.isclose(trail_stop, 960)  # 1000 - 2*20

    # Price moves up, trail stop should tighten
    trail_stop = atr_trailing_stop(
        price=1010, atr=20, trail_mult=2.0, side="long", last_trail=trail_stop
    )
    assert np.isclose(trail_stop, 970)  # 1010 - 2*20

    # Price moves down, trail stop should NOT loosen
    trail_stop = atr_trailing_stop(
        price=990, atr=20, trail_mult=2.0, side="long", last_trail=trail_stop
    )
...
def test_atr_trailing_stop_short():
    ...
    # Initial stop
    trail_stop = atr_trailing_stop(
        price=500, atr=10, trail_mult=3.0, side="short", last_trail=None
    )
    assert np.isclose(trail_stop, 530)  # 500 + 3*10

    # Price moves down, trail stop should tighten
    trail_stop = atr_trailing_stop(
        price=490, atr=10, trail_mult=3.0, side="short", last_trail=trail_stop
    )
    assert np.isclose(trail_stop, 520)  # 490 + 3*10

    # Price moves up, trail stop should NOT loosen
    trail_stop = atr_trailing_stop(
        price=510, atr=10, trail_mult=3.0, side="short", last_trail=trail_stop
    )
